---
title: "Orthographic prediction error: Functions and data"
...
library(EBImage) #Version: 4.18.1
library(vwr) #Version: 0.3.0
library(remef) #Version: 1.0.6.90
...
#---------------- Estimate prediction image ----------------
#input: list of character strings from a lexicon

getPREDICTION = function(words, font = "mono"){
  
  pred = NULL
  target = NULL
  for (ii in 1:length(words)) {  

    # save a temporary image with the printed string
    png(filename = paste(getwd(),"/tmp.png",sep=''), width = 200, height = 40, 
        units = "px", pointsize = 20, bg = "white", res = 120)
    par(mar=c(0,0,0,0))
    plot(c(0,1), axes=F, main="", xlab="" ,ylab="", col="white")
    text(.95,0.42, words[ii], pos=4,  family=font, font=2)
    dev.off()
    
    #read the temporary image
    img = readImage(paste(getwd(),"/tmp.png",sep=''))
    #reduce dimensions
    img_t<-imageData(img)[,,1]
    
    #first step of prediction estimation: addition of pixel valuse
    if (ii == 1){
      target = img_t
    }
    else{target = target + img_t}

  }
  #second step: normalization
  pred = target/length(words)
...
#---------------- Estimate oPE ----------------
#input: list of character strings, prediction image from the getPREDICTION function

getoPE = function(words, prediction, font = "mono"){
  ...
  for (ii in 1:length(words)) {  
    ...
    #prediction error image estimation
    target = abs(img_t-prediction)
...
getNPIX = function(words, font = "mono"){
  ...
  for (ii in 1:length(words)) {  
    ...
    #invert image values
    target = abs(img_t*-1+1)
    #sum the non-white pixels, note gray will be counted partely depending on thier gray values
    lum[ii] = sum(target)
  } 
  return(lum)
}
...
```{r}
#Toy example of estimating the oPE and the Number of pixels

wordlist = c("Hello","World")

#Calculate prediction
prediction = getPREDICTION(wordlist, font = "mono")
#show prediction based on "Hello" and "World"
display(prediction)

#Calculate oPE and Npixel
paste("oPE for 'Hello':",getoPE("Hello",prediction))
paste("N pixel estimate for 'Hello':",getNPIX("Hello"))
...
```{r}
#Correlation of predictions from different languages
#This can take time

pred_ger = getPREDICTION(dat_ger_5$strings) 
pred_br = getPREDICTION(dat_eng_5$Word)
pred_fr = getPREDICTION(dat_fr_5$Word)
pred_du = getPREDICTION(dat_du_5$Word)
pred_du2 = getPREDICTION(du2_wo_char$Woord)


pred_ger_v = c(pred_ger)
pred_br_v = c(pred_br)
pred_fr_v = c(pred_fr)
pred_du_v = c(pred_du)
pred_du_v2 = c(pred_du2)
pred_br_prop_v = c(pred_ger_tnrb)

cor_df = data.frame(pred_ger_v,pred_br_v,pred_fr_v,pred_du_v,pred_du_v2,pred_br_prop_v)

cor(cor_df)
```

```{r}
# Estimate prediction based on Timen new roman font 
pred_ger_tnrb = getPREDICTION(dat_ger_5$strings, font = "Times New Roman")
display(pred_ger_tnrb)

pred_ger_tnrb_v = c(pred_ger_tnrb)
cor(pred_ger_tnrb_v,pred_ger_v)
```

```{r}
#Lexial decision behaviour
dat_lexdec = read.csv("./ger_tr_e1_v01.csv",header = T)
summary(dat_lexdec)
